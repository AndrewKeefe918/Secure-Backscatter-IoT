...
from .gui_setup_secure import BasebandWindow, CarrierWindow, FskWindow
...
class ReceiverLoop:
    ...
    def __init__(
        ...
        # SECURE MODE: payload is per-packet random ciphertext, so we don't
        # compare it bitwise. The MAC is the integrity check, computed by
        # SecureReceiver from the 16 raw payload bytes after the header.
        ...
        for decode_offset in range(config.REPETITION_CHIPS):
            ...
            def _scan_with_header(
                ...
                while True:
                    ...
                    # SECURE MODE: hand the 16 raw bytes to the AEAD layer.
                    # The MAC failure rate replaces the old payload_errors
                    # heuristic. There is no "polarity" inversion because
                    # ciphertext bits don't match any known pattern.
