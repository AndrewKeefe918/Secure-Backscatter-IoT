...
This section shows the equations that tie the exciter, receiver, and tag
together. All three subsystems must agree on the same RF, IF, and bit-rate
constants — anything below that is "shared" must match in every file.
...
**Observed LO offset between Plutos.** Each PlutoSDR has its own ~25 MHz
reference oscillator with a typical accuracy of $\pm 5$ ppm. At
$f_{LO} = 2.48$ GHz this allows up to
$\pm 5\ \text{ppm}\cdot 2.48{\times}10^9 \approx \pm 12.4$ kHz of
mismatch between the exciter's TX LO and the receiver's RX LO, so the
actual peak frequency seen by the receiver is

$$
f_{peak,RX} = f_{TONE} + (f_{LO,TX} - f_{LO,RX})
$$

In practice the receiver's argmax-based peak finder locks on the
exciter tone wherever it appears (commonly several kHz off the nominal
+15 625 Hz design point) and slowly tracks its thermal drift; the CFO
estimator then derotates the IQ stream to that observed peak before
chip slicing.
...
$$
t_{rx} + t_{proc} > T_{buf}
$$

A frame is also flagged as a "gap slip" when consecutive frame starts
exceed $1.5\,T_{buf}$, indicating the OS scheduler stalled the loop.
...
$$
T_{buf} = \frac{N_{buf}}{f_s}
$$

With `RX_BUFFER_SIZE = 65536` and `SAMPLE_RATE = 1{\times}10^{6}`
([receiver/config.py](receiver/config.py)):

$$
T_{buf} = \frac{65536}{1{\times}10^{6}} \approx 65.54\ \text{ms}
$$
...
covers the worst-case $\pm 12.4$ kHz LO mismatch from §1.
...
### 10. End-to-end constant alignment
