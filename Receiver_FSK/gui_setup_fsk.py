"""Matplotlib figure and artist initialisation for the receiver GUI — FSK version.
...
def setup_baseband_window() -> BasebandWindow:
    ...
    fig.canvas.manager.set_window_title("Pluto Baseband Receiver — FSK")
...
def setup_carrier_window() -> CarrierWindow:
    ...
    carrier_fig.canvas.manager.set_window_title("Pluto Carrier Detail — FSK")
    carrier_fig.suptitle(
        f"Auto-Centered Carrier — FSK markers at "
    ...
    ax_centered.axvline(
        ...
        linestyle="--", label=f"±{config.FSK_F1_HZ/1000:.1f} kHz ('1')",
    )
    ...
    ax_centered.axvline(
        ...
        linestyle="--", label=f"±{config.FSK_F0_HZ/1000:.1f} kHz ('0')",
    )
...
def setup_fsk_window() -> FskWindow:
    """FSK demodulator window — replaces the OOK NCC window.
    ...
    fsk_fig.suptitle(
        f"FSK Demodulation — m({config.FSK_F1_HZ/1000:.1f} kHz) vs m({config.FSK_F0_HZ/1000:.1f} kHz)"
    )
